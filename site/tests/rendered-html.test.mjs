...
for (const [pathname, language, title] of [
  ...
  test(`server-renders the ${language} playbook`, async () => {
    ...
    assert.match(html, /Work mode|Mode de travail/i);
    assert.match(html, /What is connected|Qu’est-ce qui est connecté/i);
    ...
    assert.match(html, /Load a demonstration result|Charger un résultat de démonstration/i);
    assert.match(html, /coherent fictional pilot|pilote fictif cohérent/i);
    assert.match(renderedText, /0\/20/);
    ...
    assert.match(html, /RELEASE BLOCKED|MISE EN SERVICE BLOQUÉE/i);
    assert.match(html, /ROLLBACK IN FIVE PROVABLE STEPS|RETOUR ARRIÈRE EN CINQ ÉTAPES VÉRIFIABLES/i);
    assert.match(renderedText, /Not set|Non définie/i);
    ...
    assert.match(html, /Guided start|Départ guidé/i);
