...
for (const [pathname, language, title] of [
  ...
  test(`server-renders the ${language} playbook`, async () => {
    ...
    assert.match(html, /Six gates before public production|Six gates avant la production publique/i);
    assert.match(renderedText, /60–80%|60–80 %/i);
    assert.match(renderedText, /18[.,]5 h → 16 min|18\.5h → 16m/i);
    assert.match(html, /low case fails the economic gate|cas bas échoue au gate économique/i);
