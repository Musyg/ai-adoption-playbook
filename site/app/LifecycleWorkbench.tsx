...
const phaseDefinitions: Record<Locale, PhaseDefinition[]> = {
  en: [
    ...
    { code: "6", title: "Evaluations", short: "Write failure first", question: "Which test cases and failure limits would tell you the system is not ready?", why: "Tests written after a demo tend to confirm what already looked good.", evidence: "Versioned evaluation set, segments, thresholds, and stop rules.", fields: [field("evaluationCases", "Frozen cases", "Include normal, rare, critical, adversarial, and missing-data cases.", "number"), field("valueFloor", "Minimum value improvement (%)", "Measure accepted outcomes or human time, not model activity.", "number"), field("qualityFloor", "Minimum accepted quality (%)", "Define the review and major-correction budget.", "number"), field("criticalSegments", "Critical segments", "Languages, groups, channels, products, or rare conditions.", "textarea"), field("stopRule", "Critical stop rule", "One critical unauthorized or unsafe effect should be enough.", "textarea")] },
  ...
  fr: [
    ...
    { code: "1", title: "Situation initiale", short: "Mesurer avant l’IA", question: "Que se passe-t-il aujourd’hui sur l’ensemble de la charge ?", why: "Comptez toutes les demandes et leur temps actuel. Sinon, une tâche plus rapide peut donner l’illusion d’un gain sur toute la charge.", evidence: "Situation initiale datée avec volume, temps humain, résultat, erreurs et exclusions.", fields: [field("baselineVolume", "Cas par mois", "Comptez toutes les demandes, y compris le travail exclu.", "number"), field("baselineMinutes", "Minutes humaines par cas", "Utilisez si possible le temps actif observé.", "number"), field("baselineOutcome", "Unité de réussite actuelle", "Exemple : devis accepté, demande résolue, dossier complet."), field("baselineErrors", "Taux actuel d’erreur ou de reprise (%)", "Gardez la même définition pendant le pilote.", "number")] },
...
export function LifecycleWorkbench(props: Props) {
  ...
  const securityControls = useMemo(() => {
    ...
    if ((values.externalEffects && values.externalEffects !== "none") || props.integrationId !== "copilot" || props.usePatternId === "agentic" || props.autonomy >= 2) {
    ...
    if (props.autonomy >= 3) rows.push({ id: "SEC-AUTONOMY", en: "Written permission, budget, duration, destination, exception, and stop boundaries", fr: "Limites écrites de permission, budget, durée, destination, exception et arrêt" });
    if (props.autonomy >= 4) rows.push({ id: "SEC-A4", en: "Executive exception, independent review, stronger containment, and evidence that A3 is insufficient", fr: "Exception de direction, revue indépendante, confinement renforcé et preuve qu’A3 ne suffit pas" });
    ...
    for (const control of props.matchedControls.filter((item) => item.phases.includes(7))) rows.push({ id: control.id, en: control.title, fr: control.title });
    return [...new Map(rows.map((row) => [row.id, { id: row.id, label: row[locale] }])).values()];
  }, [derivedRisk, locale, props.architectureId, props.autonomy, props.integrationId, props.matchedControls, props.usePatternId, values.dataSensitivity, values.externalEffects, values.externalInteraction, values.knowledgeSource]);
  ...
  return (
    <div className="lifecycle-workbench" id="lifecycle-workbench">
      ...
      <section aria-labelledby="lifecycle-phase-title" className="lifecycle-phase" data-phase={current.code}>
        ...
        {activePhase === 1 && baselineHours > 0 && <output className="lifecycle-result"><span>{labels.hours}</span><strong>{baselineHours.toFixed(1)} h</strong><p>{locale === "en" ? "Include every request, even failures and manual work, when calculating a later saving." : "Incluez toutes les demandes, même les échecs et le travail manuel, pour calculer un gain ultérieur."}</p></output>}
